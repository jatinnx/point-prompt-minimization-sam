...
from .config import SAM3_HUB_ID, SAM3_LOCAL_DIR, Config
...
def build_backend(cfg: Config, verbose: bool = True) -> SamBackend:
    """Build the SAM 3 backend, or raise ``BackendUnavailable`` saying why.

    A function rather than a bare constructor because the harness builds lazily
    and everything that only reads regions, scores or the stored hand-off must
    keep working on a machine with no weights at all.
    """
    backend = Sam3Backend(cfg)
    if verbose:
        print(f"[backend] using {backend.name} on {cfg.device} "
              f"from {cfg.sam3_model_id}")
    return backend
